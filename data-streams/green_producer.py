...
def stream_data():
    ...
    file_path = "datasets/taxi/green_tripdata_2026-03.parquet"
    if not os.path.exists(file_path):
        print(f"[ERROR] File not found: {file_path}")
        return

    print(f"[{TAXI_TYPE.upper()} PRODUCER] Pre-loading data from {file_path}...")
    cols_needed = ["VendorID", "lpep_pickup_datetime", "lpep_dropoff_datetime", 
    ...
    while True:
        ...
        for _, row in df.iterrows():
            ...
            time.sleep(0.005) # Green moves a bit slower but loops faster due to small size
