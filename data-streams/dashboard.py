...
def update_dashboard():
    ...
    try:
        ...
        df_counts = pd.read_sql(
            "SELECT (SELECT COUNT(*) FROM normal_trips) AS total_normal, (SELECT COUNT(*) FROM anomalous_trips) AS total_anomalies",
            conn
        )
        total_normal = int(df_counts['total_normal'].iloc[0])
        total_anomalies = int(df_counts['total_anomalies'].iloc[0])
    ...
    if not df_anomalies.empty:
        ...
        st.dataframe(display_df.style.map(lambda _: "background-color: #551111; color: white;"), use_container_width=True)
    else:
